...
def upgrade():
    op.create_table(
        ...
        sa.Column('from_date', sa.DateTime, nullable=False, server_default=sa.func.current_timestamp()),
        sa.Column('to_date', sa.DateTime, nullable=False, server_default=sa.func.current_timestamp()),
        sa.Column('message', sa.UnicodeText, nullable=True),
        sa.Column('status', sa.Enum('draft', 'active', 'deleted', name='announcements_status_enum'), nullable=False),
